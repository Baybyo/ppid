...
<div class="page-header">
  <h1><i class="bi bi-person-gear" style="color:var(--blue-600);"></i> Profil Saya</h1>
  <p>Kelola data akun dan keamanan akun admin</p>
</div>
  ...
  <div class="profile-sidebar">
    <div class="profile-card">
      <div class="profile-avatar-lg">
        <?= esc(strtoupper(substr($admin['nama'] ?? 'AD', 0, 2))) ?>
      </div>
      <h3 class="profile-name"><?= esc($admin['nama'] ?? '-') ?></h3>
      <p class="profile-email">@<?= esc($admin['username'] ?? '-') ?></p>
    ...
    <div class="tab-content active" id="tab-profil">
      <div class="profile-section">
        <div class="section-header">
          <h4><i class="bi bi-person-lines-fill"></i> Informasi Akun</h4>
          <p>Perbarui data akun admin Anda</p>
        </div>
        <form action="<?= site_url('admin/profil/update') ?>" method="post" class="profile-form">
          <?= csrf_field() ?>
          <div class="form-row form-row-2">
            <div class="form-group">
              ...
              <div class="input-icon">
                ...
                <input type="text" name="nama" id="nama" value="<?= esc($admin['nama'] ?? '') ?>" required minlength="3" placeholder="Masukkan nama lengkap">
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="input-icon">
                <i class="bi bi-at"></i>
                <input type="text" name="username" id="username" value="<?= esc($admin['username'] ?? '') ?>" required placeholder="Username login">
              </div>
            </div>
          </div>
...
<style>
  ...
  .profile-card { background: var(--white); border: 1px solid var(--n-200); border-radius: var(--r); padding: 1.75rem 1.25rem; text-align: center; box-shadow: var(--shadow-sm); }
  .profile-avatar-lg { width: 80px; height: 80px; border-radius: 50%; background: var(--blue-600); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.75rem; font-weight: 800; margin: 0 auto .75rem; box-shadow: 0 0 0 4px var(--blue-100); }
  .profile-name { font-size: 1.05rem; font-weight: 700; color: var(--n-900); margin: 0 0 .2rem; }
  .profile-email { font-size: .78rem; color: var(--n-400); margin: 0 0 .75rem; }
  .profile-badge { display: inline-flex; align-items: center; gap: .35rem; background: var(--blue-100); color: var(--blue-700); padding: .3rem .7rem; border-radius: 20px; font-size: .7rem; font-weight: 600; }
  ...
  .profile-tabs { display: flex; gap: 0; border-bottom: 2px solid var(--n-100); margin-bottom: 1.25rem; }
  .profile-tab { padding: .65rem 1.25rem; font-size: .82rem; font-weight: 600; color: var(--n-400); background: none; border: none; border-bottom: 2px solid transparent; margin-bottom: -2px; cursor: pointer; transition: all .15s var(--ease); display: flex; align-items: center; gap: .4rem; }
  .profile-tab:hover { color: var(--n-600); }
  .profile-tab.active { color: var(--blue-600); border-bottom-color: var(--blue-600); }
  .tab-content { display: none; }
  .tab-content.active { display: block; }
  .profile-section { background: var(--white); border: 1px solid var(--n-200); border-radius: var(--r); padding: 1.5rem; box-shadow: var(--shadow-sm); }
  ...
  .profile-form label { display: block; font-size: .78rem; font-weight: 600; color: var(--n-600); margin-bottom: .35rem; }
  .required { color: var(--red-500); }
  .input-icon { position: relative; display: flex; align-items: center; }
  .input-icon i { position: absolute; left: .75rem; color: var(--n-400); font-size: .9rem; pointer-events: none; }
  .input-icon input { width: 100%; padding: .55rem .75rem .55rem 2.25rem; border: 1px solid var(--n-200); border-radius: 8px; font-size: .82rem; color: var(--n-800); transition: border-color .15s, box-shadow .15s; background: var(--white); }
  .input-icon input:focus { outline: none; border-color: var(--blue-400); box-shadow: 0 0 0 3px var(--blue-100); }
  ...
  @media (max-width: 768px) { .profile-grid { grid-template-columns: 1fr; } .profile-form .form-row-2 { grid-template-columns: 1fr; } }
</style>
